...
var entertainmentNews = document.querySelectorAll(".entertainment .news div");
var sportsNews = document.querySelectorAll(".sports .news div");
...
var API_KEY1 = "fc48dea66b48d8bcb20c527e0d13e7ae";
...
var newsSportURL = `https://gnews.io/api/v4/top-headlines?category=sports&lang=ar&country=eg&apikey=${API_KEY1}`;
var newsEntertainmentURL = `https://gnews.io/api/v4/top-headlines?category=entertainment&lang=ar&country=eg&apikey=${API_KEY1}`;
...
// Entertainment News
$.ajax({
    url: newsEntertainmentURL,
    dataType: 'json',
    type: 'GET',
    success: function (response) {
        console.log("Entertainment news response ",response);
        if (response && response.articles) {
            for (var i = 0; i < 3; i++) {
                if (entertainmentNews[i] && response.articles[i]) {
                    setupNewsCard(
                        entertainmentNews[i],
                        response.articles[i].image,
...
$.ajax({
    ...
    type: 'GET',
    success: function (response) {
        console.log("sport news response",response);
        if (response && response.articles) {
            for (var i = 0; i < 3; i++) {
                if (sportsNews[i] && response.articles[i]) {
                    setupNewsCard(
                        sportsNews[i],
                        response.articles[i].image,
                        response.articles[i].title
